...
//Pages
	Page:function(){
		...
		function ctrl(pos){
			return '<a onclick="DMController.done('+pos+',this)" class="btn btn-xs btn-default"><span class="glyphicon glyphicon glyphicon-trash"></span></a>';
		}
		this.html('<h2>Liste des DM :</h2>'+
			'<table class="table table-condensed">'+
			'<thead><tr><td>#</td><td>Titre</td><td>UE</td><td>Date</td><td width="1%"></td></tr></thead>'+
			'<tbody>'+
			dms.map(function(dm,pos){if(!dm)return '';//previously deleted entry
				return '<tr class="'+(dm.done?'active':['success','info','warning','danger'][dm.lv])+'"><td>'+
					[pos,dm.title,dm.UE,(new Date(dm.date)).toLocaleDateString(),ctrl(pos)].join('</td><td>')+'</td></tr>';
			}).join('')+
			'</tbody></table>');
	}
}